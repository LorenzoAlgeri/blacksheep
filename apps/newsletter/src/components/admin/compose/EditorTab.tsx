...
import { Send, Plus, X, Upload, RotateCcw, Clock, Link2, User } from "lucide-react";
import { PALETTE_PRESETS } from "@/lib/email-template";
import { buildEventRegistrationUrl, insertAtCursor } from "@/lib/email-campaign";
import { EventPickerDialog, type PickableEvent } from "@/components/admin/EventPickerDialog";
...
export function EditorTab({
  ...
  // Track cursor position so the event link inserts at the right spot
  ...
  function handleEventSelect(event: PickableEvent) {
    ...
    set("body", newBody);
    // Restore focus and cursor position after the inserted URL.
    const newPos = cursorPos + url.length;
    setTimeout(() => {
      if (bodyRef.current) {
  ...
  function handleInsertName() {
    ...
    const newBody = insertAtCursor(state.body, placeholder, cursorPos);
    set("body", newBody);
      ...
      <div className="flex flex-col gap-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <label htmlFor="nl-body" className={labelClass + " mb-0"}>
              Messaggio
            </label>
            {/* Toolbar: insert helpers for the body textarea */}
            <div className="flex gap-1.5">
              ...
              <button
                ...
                title="Inserisce {{name}} — variabile nome iscritto (richiede supporto backend)"
                className="flex items-center gap-1 font-[family-name:var(--font-brand)] text-[9px] tracking-[0.1em] text-bs-cream/40 hover:text-bs-cream/70 border border-bs-cream/10 hover:border-bs-cream/25 px-2 py-1 rounded transition-colors cursor-pointer"
              >
          ...
          <textarea
            ...
            onMouseUp={handleBodyCursorChange}
            onBlur={handleBodyCursorChange}
