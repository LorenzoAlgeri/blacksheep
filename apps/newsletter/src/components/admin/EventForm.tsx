  ...
  const onSubmit = async (data: EventFormValues) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const savedEventId = resBody?.event?.id ?? eventId;

        // Detect if this action published the event (new publish or status change to published)
        const wasPublished = data.status === "published" && defaultValues?.status !== "published";
  ...
  return (
    <>
      ...
      {pushDialog?.show && (
        ...
        >
          <div className="bg-[#0a1a3a] border border-bs-cream/20 rounded-lg p-6 max-w-sm mx-4 w-full">
            <h3
              id="push-dialog-title"
              className="font-[family-name:var(--font-brand)] text-sm text-bs-cream tracking-wider mb-3"
            >
              INVIA NOTIFICA PUSH
            </h3>
            {pushResult ? (
              <>
                <p className="font-body text-xs text-bs-cream/60 mb-4">{pushResult}</p>
                <button
                  type="button"
                  onClick={() => {
                    setPushDialog(null);
                  ...
                  className="font-body text-xs tracking-wider px-4 py-2 rounded border border-bs-cream/20 text-bs-cream/60 hover:text-bs-cream transition-colors cursor-pointer"
                >
                  CHIUDI
                </button>
              </>
              ...
              <>
                <p className="font-body text-xs text-bs-cream/50 mb-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="font-body text-xs text-bs-cream/40 hover:text-bs-cream transition-colors cursor-pointer"
                  >
                    NO, SALTA
                  </button>
                </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
        ...
        <div>
          <label htmlFor="event-capacity" className={labelClass}>
            CAPIENZA{" "}
            <span className="text-bs-cream/30 normal-case tracking-normal">(opzionale)</span>
          </label>
          <input
            id="event-capacity"
            type="number"
