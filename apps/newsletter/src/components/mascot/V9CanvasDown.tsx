...
/** V9-down — canvas + ImageBitmap, but with backing HALF the source
 *  resolution (960×540) and drawImage downscale. The browser then
 *  upsamples 960×540 backing to viewport size with bilinear smoothing
 *  → pixel-bleeding around bright highlights = "luccichio" glow.
 *  This is the leading hypothesis for the V4-original effect. */
export function MascotteIntroV9CanvasDown() {
  ...
  useEffect(() => {
    ...
    let revealed = false;
    const SCALE = 0.5; // backing 960×540 from 1920×1080
    ...
    const playFrom = (startTime: number) => {
      ...
      const backingW = Math.round(intrinsicW * SCALE);
      const backingH = Math.round(intrinsicH * SCALE);
      ...
      const tick = (now: number) => {
        ...
        if (bm) {
          ...
          ctx.drawImage(bm, 0, 0, backingW, backingH);
        }
