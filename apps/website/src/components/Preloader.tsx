...
// Check sessionStorage synchronously to avoid flash / setState-in-effect
...
function useAlreadyShown() {
  return useSyncExternalStore(noop, getSessionShown, () => false);
}
...
export function Preloader({ onComplete }: { onComplete: () => void }) {
  ...
  const shouldShow = !prefersReduced && !alreadyShown;
  const completedRef = useRef(false);
  const [visible, setVisible] = useState(shouldShow);

  // Immediately signal completion if we shouldn't show
  useEffect(() => {
    if (!shouldShow && !completedRef.current) {
      completedRef.current = true;
      onComplete();
    }
  }, [shouldShow, onComplete]);

  // Run preloader animation (shortened to ~1.2s total)
  useEffect(() => {
    ...
    const done = () => {
      ...
      if (!completedRef.current) {
        completedRef.current = true;
        setVisible(false); // Remove from DOM so it doesn't block LCP
        onComplete();
      }
    };
    ...
    // Loading bar fills (0.4s)
    tl.to(bar, { width: "100%", duration: 0.4, ease: EASE.move });

    // Stop pulse + logo scale-up (0.15s)
    tl.add(() => {
      pulse.kill();
    });
    tl.to(logo, { scale: 1.15, duration: 0.15, ease: EASE.enter });

    // Overlay slides up (0.35s)
    tl.to(overlay, {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path d={LOGO_PATH_INNER} />
      </svg>

      {/* Loading bar */}
      <div className="mt-6 h-0.5 w-24 bg-bs-cream/10 overflow-hidden">
        <div ref={barRef} className="h-full bg-bs-cream" style={{ width: "0%" }} />
      </div>
    </div>
  );
}
